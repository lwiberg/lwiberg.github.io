...
// Preserve visible content during responsive resizes
(function() {
  let resizeTimer = null;
  let focusedElement = null;
  let savedScrollRatio = null;

  function pickFocusedElement() {
    // Prefer element near vertical center of viewport
    const x = Math.floor(window.innerWidth / 2);
    const y = Math.floor(window.innerHeight / 2);
    let el = document.elementFromPoint(x, y);
    if (!el) return null;
    // find the nearest media/grid item ancestor
    return el.closest('.nine-item-grid img, .nine-item-grid video, .five-item-collage img, .video-row > *, .grid img');
  }

  function saveStateBeforeResize() {
    focusedElement = pickFocusedElement();
    const docHeight = Math.max(document.documentElement.scrollHeight - window.innerHeight, 1);
    savedScrollRatio = window.scrollY / docHeight;
  }

  function restoreStateAfterResize() {
    if (focusedElement && document.body.contains(focusedElement)) {
      try {
        focusedElement.scrollIntoView({ block: 'center', inline: 'nearest' });
      } catch (e) {
        // fallback to ratio method below
        focusedElement = null;
      }
    }

    if (!focusedElement && savedScrollRatio !== null) {
      const docHeight = Math.max(document.documentElement.scrollHeight - window.innerHeight, 1);
      window.scrollTo(0, Math.round(savedScrollRatio * docHeight));
    }
  ...
  window.addEventListener('resize', function() {
    // on first resize event, capture what's visible
    if (!resizeTimer) saveStateBeforeResize();

    clearTimeout(resizeTimer);
    resizeTimer = setTimeout(function() {
      restoreStateAfterResize();
      resizeTimer = null;
    }, 150);
  });
})();
